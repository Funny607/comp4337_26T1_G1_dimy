...
from src.network_tcp import TCPBackendClient
...
BackendResultHandler = Callable[[dict[str, Any]], None]
...
class DimyNode:
    def __init__(
        ...
        backend_result_handler: Optional[BackendResultHandler] = None,
    ) -> None:
        self.config = config
        self.encounter_handler = encounter_handler
        self.qbf_handler = qbf_handler
        self.backend_result_handler = backend_result_handler
        ...
        self.last_backend_result: Optional[dict[str, Any]] = None
        ...
        self.backend_client: Optional[TCPBackendClient] = None
        if self.config.enable_tcp_backend:
            self.backend_client = TCPBackendClient(
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self._epoch_thread = threading.Thread(target=self._epoch_loop, name=f"DimyEpochLoop:{self.config.node_id}", daemon=True)
            self._broadcast_thread = threading.Thread(target=self._broadcast_loop, name=f"DimyBroadcastLoop:{self.config.node_id}", daemon=True)
            self._epoch_thread.start()
            self._broadcast_thread.start()

            if self.config.enable_qbf_scheduler:
                self._qbf_thread = threading.Thread(target=self._qbf_loop, name=f"DimyQBFLoop:{self.config.node_id}", daemon=True)
                self._qbf_thread.start()

            self._debug(
                f"node started; epoch={current_epoch} t={self.config.t} "
                f"k={self.config.k} n={self.config.n} p={self.config.p}%"
            )
    ...
    def ensure_epoch(self, epoch_id: Optional[int] = None) -> LocalEpochState:
        ...
        with self._lock:
            ...
            state = LocalEpochState(epoch_id=epoch_id, created_at=time.time(), ephid=ephid, shares=shares)
            self.local_epochs[epoch_id] = state
            self._prune_old_state_locked()

        self._debug(f"new EphID epoch={epoch_id} ephid={short_hex(state.ephid_bytes)} hash={state.ephid_hash[:12]}...")
        ...
        while self._running.is_set():
            try:
                ...
                if period_id != last_period_id:
                    ...
                    if qbf is not None:
                        if self.qbf_handler is not None:
                            self.qbf_handler(qbf)
                        if self.backend_client is not None and self.config.auto_query_backend and not self._cbf_uploaded:
    ...
    def _prune_old_state_locked(self) -> None:
        ...
        if epoch_ids:
            ...
            stale_keys = [key for key in self.peer_buffers.keys() if key[0] < min_epoch_to_keep]
    ...
    def send_next_share(self) -> None:
        ...
        if self.transport is None:
            self._debug(f"(no UDP) prepared share epoch={state.epoch_id} share_index={share.index}/{self.config.n}")
            return
        self.transport.send_packet(packet)
        self._debug(
            f"broadcast share epoch={state.epoch_id} share_index={share.index}/{self.config.n} "
    ...
    def handle_udp_packet(self, packet: dict[str, Any], addr: tuple[str, int]) -> None:
        if not isinstance(packet, dict) or packet.get("type") != "share":
            return
        try:
        ...
        with self._lock:
            ...
            if share.index in buffer.shares_by_index:
                self._debug(f"duplicate share ignored sender={sender_id} epoch={epoch_id} share_index={share.index}")
                return
        ...
        self._debug(
            f"received share sender={sender_id} epoch={epoch_id} share_index={share.index}/{n} total_distinct={share_count}"
        )
        if share_count >= k:
    ...
    def _attempt_reconstruction(self, buffer_key: tuple[int, str, str]) -> None:
        with self._lock:
            buffer = self.peer_buffers.get(buffer_key)
            if buffer is None or buffer.reconstructed or len(buffer.shares_by_index) < buffer.k:
                return
            shares = list(buffer.shares_by_index.values())[:buffer.k]
            local_state = self.local_epochs.get(buffer.epoch_id)
            if local_state is None:
                self._debug(
                    f"cannot reconstruct sender={buffer.sender_id} epoch={buffer.epoch_id}: no matching local epoch retained"
                )
                return
        ...
        try:
            ...
            self._debug(f"reconstruction failed sender={buffer.sender_id} epoch={buffer.epoch_id}: {exc!r}")
            return

        if not verify_ephid_hash(reconstructed_ephid, buffer.ephid_hash):
            self._debug(f"reconstruction hash mismatch sender={buffer.sender_id} epoch={buffer.epoch_id}")
            return

        self._debug(
            f"reconstruction OK sender={buffer.sender_id} epoch={buffer.epoch_id} ephid={short_hex(reconstructed_ephid)}"
        )
        ...
        try:
            ...
            self._debug(f"EncID derivation failed sender={buffer.sender_id} epoch={buffer.epoch_id}: {exc!r}")
            return
        ...
        with self._lock:
            ...
            if encid_hex in self.seen_encounter_ids:
                ...
                self._debug(f"duplicate EncID ignored sender={buffer.sender_id} epoch={buffer.epoch_id}")
                return
            self.seen_encounter_ids.add(encid_hex)
            buffer.reconstructed = True

        self._debug(f"EncID derived sender={buffer.sender_id} epoch={buffer.epoch_id} encid={short_hex(encid)}")
    ...
    def _store_encounter_in_dbf(self, encid: bytes) -> DBFState:
        ...
        self._debug(f"EncID encoded into DBF window={dbf.window_id} encounters={dbf.encounter_count}")
        return dbf
    ...
    def _emit_encounter(self, encid: bytes, metadata: dict[str, Any]) -> None:
        ...
        if self.encounter_handler is not None:
            ...
            self._debug(
                f"encounter stored; EncID deleted after DBF insert sender={metadata.get('sender_id')} encid={short_hex(encid)}"
            )

    def upload_current_cbf(self) -> dict[str, Any]:
        if self.backend_client is None:
            raise RuntimeError("TCP backend is not enabled")
        cbf = self.dbf_manager.build_cbf()
        result = self.backend_client.upload_cbf(
            node_id=self.config.node_id,
            cbf_bytes=cbf.to_bytes(),
            size_bytes=self.config.filter_size_bytes,
            hash_count=self.config.hash_count,
        )
        if result.get("ok") is True and result.get("result") == "uploaded":
            self._cbf_uploaded = True
            self.dbf_manager.set_qbf_enabled(False)
        self.last_backend_result = result
        self._emit_backend_result(result)
        return result
    ...
    def query_backend_for_qbf(self, qbf: Optional[QBFState] = None) -> dict[str, Any]:
        if self.backend_client is None:
            raise RuntimeError("TCP backend is not enabled")
        if qbf is None:
            qbf = self.dbf_manager.get_last_qbf() or self.dbf_manager.build_qbf(force=True)
        if qbf is None:
            raise RuntimeError("QBF is not available")
        result = self.backend_client.query_qbf(
            node_id=self.config.node_id,
    ...
    def _emit_backend_result(self, result: dict[str, Any]) -> None:
        self._debug(f"backend result={result.get('result')} ok={result.get('ok')}")
        if self.backend_result_handler is not None:
            self.backend_result_handler(result)
